...
class Recall(EvaluationABC):
    ...
    def evaluate(self,
        ...
        recall_ovr = recall_score(y_test, y_pred, average=average)

        from src.mlops.evaluation import evaluate_outsample_ovo
        metric = 'recall_score'
        rs_ovo_01 = evaluate_outsample_ovo(metric, y_test, y_pred, y_proba, class_a=0, class_b=1)
        rs_ovo_02 = evaluate_outsample_ovo(metric, y_test, y_pred, y_proba, class_a=0, class_b=2)
        ...
        with mlflow.start_run(run_id=mlflow_model_run_id, nested=True):
            mlflow.log_metric(f"outsample_ovr_recall_{average}", recall_ovr)
            mlflow.log_metric(f"outsample_class0v1_recall", rs_ovo_01)
            mlflow.log_metric(f"outsample_class0v2_recall", rs_ovo_02)