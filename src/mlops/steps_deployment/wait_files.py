...
def wait_for_files(econ: int,
    ...
    while time.time() - start_time < timeout_sec:
        try:
            ...
            print(f"⚠️ SMB access error: {e}")
        time.sleep(poll_interval_sec)

    if not folder_name:
        raise TimeoutError(f"⛔ Timeout: Folder for date {date} not found in {base_dir}")
    ...
    for path in [x_path, y_path]:
        ...
        while time.time() - start_time < timeout_sec:
            ...
            raise TimeoutError(f"⛔ Timeout: File not found - {path}")
